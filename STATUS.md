...
- Extended the tour detail-panel step to desktop: step 6 now opens the detail panel with the Cuyahoga sample selection on desktop too (previously mobile-only, so desktop still highlighted the app bar), and the card clamp slides the tour card to the left of the open detail panel when the tour's fallback placement would cover it. Selection cleanup on step change and tour end matches the mobile behavior; the mobile card repositioning class stays mobile-only.
